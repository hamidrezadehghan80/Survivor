...
import {profile_cover, Who_to_follow_avatar} from "../../Constants/images";
import {Grid} from "@mui/material";
import Header from "../../Layout/Header/Header";
import {Link} from "react-router-dom";

const Followers_list = (props) => {
    ...
    return(
        <Grid container flexDirection={"column"} rowGap={"2rem"}>
            <Grid item>
                <Header title={"Followers"} arrow_f={true}/>
            </Grid>
            <Grid item container flexDirection={"column"}>
                {
                    followers_list?.map(({name, user_name, img}, index) =>
                        <Grid
                            item
                            container
                            flexDirection={"row"}
